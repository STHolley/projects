	...
	while(count < size) {
		...
		if ((val=recvfrom(sockfd,&data_msg, sizeof(data_msg), 0,(struct sockaddr *)&client, &len))<0)
			...
			if(x == data_msg.checksum){
				if(isLost(prob_loss) == true){
					printf("SERVER: Calculated checksum is (%d) --  LOST\n",x);
					printf("\t\t>>>>>>> NETWORK: ACK was not sent <<<<<<<<<\n");
				} else {
