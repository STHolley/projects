...
bool store_obj(void* obj, char* (*new_str_rep)(void*)) {
	...
	DIR* dir = opendir(OSTORE_DIR);
	if(!ostore_is_on || !dir){
		errno = ENOENT;
		return false;
	}
	closedir(dir);
	int file = open(_get_ofile_path((size_t)obj), O_CREAT | O_WRONLY);
	if(file <= 0){
		return false;
	}
	char* str = (*new_str_rep)(obj);
	write(file, str, OSTR_REP_MAX);
	int check = close(file);
	if(check == -1){
		return false;
	}
    return true;
