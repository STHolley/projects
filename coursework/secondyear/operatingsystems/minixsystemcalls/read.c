...
/* Perform read(fd, buffer, nbytes) or write(fd, buffer, nbytes) call. */
  ...
  if ((f = get_filp(scratch(fp).file.fd_nr, locktype)) == NULL){
	/* CSC2025 Mod Start */
	if(rw_flag == READING){
		logfserr_nopath(FSOP_READ, err_code);
	} else {
		logfserr_nopath(FSOP_WRITE, err_code);
	/*CSC2025 Mod End */
	return(err_code);
  }
  if (((f->filp_mode) & (rw_flag == READING ? R_BIT : W_BIT)) == 0) {
	...
	/* CSC2025 Mod Start */
	if(rw_flag == READING){
		logfserr_nopath(FSOP_READ, f->filp_mode == FILP_CLOSED ? EIO : EBADF);
	} else {
		logfserr_nopath(FSOP_WRITE, f->filp_mode == FILP_CLOSED ? EIO : EBADF);
	/*CSC2025 Mod End */
  ...
  if (scratch(fp).io.io_nbytes == 0) {
	...
	/* CSC2025 Mod Start */
	if(rw_flag == READING){
		logfsop_nopath(FSOP_READ, 0, scratch(fp).file.fd_nr, UNKNOWN_V_MODE, UNKNOWN_V_UID,
        UNKNOWN_V_GID, UNKNOWN_V_SIZE);
	} else {
		logfsop_nopath(FSOP_WRITE, 0, scratch(fp).file.fd_nr, UNKNOWN_V_MODE, UNKNOWN_V_UID,
        UNKNOWN_V_GID, UNKNOWN_V_SIZE);
	/*CSC2025 Mod End */
  ...
  /* CSC2025 Mod Start */
	if(rw_flag == READING){
		if(r == OK){
			logfsop_nopath(FSOP_READ, r, scratch(fp).file.fd_nr, UNKNOWN_V_MODE, UNKNOWN_V_UID, UNKNOWN_V_GID, UNKNOWN_V_SIZE);
		} else {
			logfserr_nopath(FSOP_READ, r);
		}
	} else {
		if(r == OK){
			logfsop_nopath(FSOP_WRITE, r, scratch(fp).file.fd_nr, UNKNOWN_V_MODE, UNKNOWN_V_UID, UNKNOWN_V_GID, UNKNOWN_V_SIZE);
		} else {
			logfserr_nopath(FSOP_WRITE, r);
		}
	}
  /*CSC2025 Mod End */
  return(r);
		      ...
		      endpoint_t for_e)
{
  ...
  if (size > SSIZE_MAX){
	  /* CSC2025 Mod Start */
	 if(rw_flag == READING){
		logfserr_nopath(FSOP_READ, EINVAL);
	  } else{
		logfserr_nopath(FSOP_WRITE, EINVAL);
	  }
	  /* CSC2025 Mod End */
	  return(EINVAL);
  }
  ...
  if (r == OK){
	  /* CSC2025 Mod Start */
	  if(rw_flag == WRITING){
		  logfsop_nopath(FSOP_WRITE, cum_io, scratch(fp).file.fd_nr, vp->v_mode, vp->v_uid,
        vp->v_gid, vp->v_size);
	  } else {
		  logfsop_nopath(FSOP_READ, cum_io, scratch(fp).file.fd_nr, vp->v_mode, vp->v_uid,
        vp->v_gid, vp->v_size);
	  }
	  /* CSC2025 Mod End */
	  return(cum_io);
  ...
  /* CSC2025 Mod Start */
  if(rw_flag == WRITING){
	  fslogerr_nopath(FSOP_WRITE, r);
  } else {
	  fslogerr_nopath(FSOP_READ, r);
  }
  /* CSC2025 Mod End */
  return(r);
}
